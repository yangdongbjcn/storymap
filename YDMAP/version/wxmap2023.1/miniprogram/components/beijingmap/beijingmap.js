// components/chinamap/chinamap.js

import * as echarts from '../../ec-canvas/echarts';
// import mapJsonDownload from './china_json_download.js';
...
Component({
  ...
  lifetimes: {
    attached: function() {
      // 在组件实例进入页面节点树时执行
      this.initMychart(this.data.dataList, this.data.dataPoint);
    },
    detached: function() {
      // 在组件实例被从页面节点树移除时执行
    },
    ready: function () {
      var _this = this
      wx.request({
        url: 'https://www.yd.bj.cn/yd/YDMAP/mapdata/china/beijing/beijing.json',//json数据地址
        // url: 'https://www.yd.bj.cn/yd/YDMAP/mapdata/china/china.json',//json数据地址
        headers: {
  ...
  methods: {
    // 设置图表所需的option
    setMychartOption(chart, dataList, dataPoint) {
      const option = {
        tooltip : {
          trigger: 'item',
    ...
    // 初始化图表
    initMychart(dataList, dataPoint) {  
      ...
      this.oneComponent.init((canvas, width, height, dpr) => {
        ...
        // echarts.registerMap('china', mapJsonDownload);
        echarts.registerMap('beijing', this.data.mapJson);
        this.setMychartOption(chart, dataList, dataPoint)  //赋值给echart图表
